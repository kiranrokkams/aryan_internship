...
def numerical_distribution_samples(var_dist, n_samples, random_state):
    ...
    if var_dist["type"] == "normal":
        y = skewnorm.rvs(a=var_dist["a"], loc=var_dist["loc"], scale=var_dist["scale"], size=n_samples, random_state=random_state)
    elif var_dist["type"] == "poisson":
        y = poisson.rvs(mu=var_dist["mu"], loc=var_dist["loc"], size=n_samples, random_state=random_state)
    elif var_dist["type"] == "custom":
        y = var_dist['func'](**var_dist['kwargs'])
    else:
        raise ValueError(
            "Please pass distribution type as one of from 'normal', 'poisson' or 'custom'!"
        )
    return y


def categorical_distribution_samples(var_dist, n_samples, generate_random_strings=False):
    ...
    if var_dist["type"] in ['probs', 'multinomial']:
        ...
        if np.sum(var_dist['probs']) != 1:
            ...
            _LOGGER.warning(
                "probabilities {} do not sum to 1, so values will be normalized as {}.".format(var_dist['probs'],
                                                                                               class_probs))
    if var_dist["type"] == "probs":
        ...
        raise ValueError(
            "Please pass distribution type as one of from 'probs', 'multinomial' or 'custom'!"
        )
    return y
...
def get_num_X_samples(n_samples, n_features_num, num_X_dist, random_state):
    ...
    if len(num_X_dist) == n_features_num:
        for i, dist in enumerate(num_X_dist):
            random_state = random_state + 1 # for getting different values for different variables
            x_cap = numerical_distribution_samples(dist, n_samples, random_state)
            x[:, i] = x_cap
    else:
        raise ValueError(
            "No of features (n_features_num) and list of numerical distributions (num_X_dist) do not match"
        )
    return x
...
def get_cat_X_samples(n_samples, cat_X_dist, random_state):
    ...
    cat_X_dist: list of dict
        custom distribution info for categorical features, each dict will corresponds to a single categorical variable
    random_state: int
        random state for random sampling
    ...
    for i, var_dist in enumerate(cat_X_dist):
        ...
        X1[f"cat_feat_{i + 1}"] = categorical_distribution_samples(var_dist, n_samples, generate_random_strings=True)
...
def random_date(start, end, frequency='d'):
    ...
    if frequency == 'd':
        ...
        raise ValueError("Invalid frequency value '{}', should be one of 'd' (Days), 'h' (Hours), 'm' (Minutes) or "
                         "'s' (Seconds).".format(frequency))
    return output


def create_data(n_samples: int, n_features_num: int, is_classification: bool, n_classes=2, target_as_func=True,
                perc_missing=0, perc_zeros=0, num_X_dist=None, cat_X_dist=None, target_dist=None, date_cols=None,
                include_id_column=False, include_id_column_unique=False, random_state=1234, **kwargs):
    ...
    target_as_func: bool, default=True
        Whether the target should be a function of X or can be picked randomly from distribution samples
    ...
    num_X_dist: list of dict, default=None
        Custom distribution info for numerical X. (Note: length of num_X_dist must be equals to n_features_num.)
        For each numeric variable (n_features_num), pass a dictionary with required parameters according to distribution
         type as follows:
        ...
        For custom dist: pass an appropriate function under 'func' key and expected arguments under 'kwargs' key
            e.g. from tweedie import tweedie
                 def func(mu, p, phi, n_samples):
                    return tweedie(mu=mu, p=p, phi=phi).rvs(n_samples)
                {"type": "custom", "func":func, "kwargs":{'mu':100, 'p':1.5, 'phi':20, 'n_samples':n_samples}}
                Note: Here, 'n_samples' value must be equals to n_samples variable passed.
        Note: If empty dictionary is passed, then default value ({"type": "normal", "a":0, "loc": 100, "scale": 5}) will
         be consider for that variable. This will be same in case 'type' key itself missing from the dictionary.
    cat_X_dist: list of dict, default=None
        Custom distribution info for categorical X., each dictionary would corresponds to generate a single categorical column.
        For each categorical variable, pass type of distribution under key 'type', optional class names under key
        ...
        Currently we support following types:
            'multinomial' - multinomial distribution defined by np.random.multinomial():
                e.g.: {'type':'multinomial', 'probs':[0.3, 0.4, 0.2, 0.1], 'class_names':['abc', 'xyz', 'pqr', 'ijk']}
            'probs' - uniform/non-uniform probabilities distribution defined by np.random.choice():
                e.g.: {'type':'probs', 'probs':[0.3, 0.4, 0.3]}
            'custom' - user can pass a direct function which should generate samples of size n_samples:
                e.g.: {'type':'custom', 'func':func, 'kwargs':{}}
        e.g. [{'type':'multinomial', 'probs':[0.3, 0.4, 0.2, 0.1]},{'type':'probs', 'probs':[0.3, 0.4, 0.3]}] would
         corresponds to generate 2 categorical columns having 4 and 3 classes (length of 'probs') respectively.
        Note: In 'multinomial' and 'probs' types:
            - Sum of 'probs' values should be 1 otherwise values will be normalized internally to make the sum as 1.
            - Optional class names can be passed under key "class_names", if passed will use the same otherwise will
              generate 5 characters random string for each category corresponds to probability specified in 'probs'.
    target_dist: dict, , default=None
        Distribution of target variable (y) to draw the random samples. For regression, it used when target_as_func=False and
        for classification, used when target_as_func=False and num_X_dist is not None.
        for regression: pass distribution info for y as expected similar for a variable in num_X_dist
            default={"type": "normal", "a":0, "loc": 100, "scale": 5}
        for classification: pass distribution info for y as expected similar for for a variable in cat_X_dist
            default={'type':'probs', 'probs':[1/n_classes]*n_classes, 'class_names':np.arrange(n_classes)}
            Note: For 'probs' and 'multinomial' type, sum of 'probs' values should be 1 otherwise values will be normalized internally.
    date_cols: list of dict, default=None
        pass a list of dictionary, each corresponding to a date column. Each dictionary would contain 'start' and 'end'
        date values in "%Y-%m-%d" format along with 'frequency' value to pick random dates within the same. Default
        value of 'start', 'end' and 'frequency' will considered as "1990-01-01", "today" and 'd' respectively in case
        when respective value is missing from the dictionary. Allowed values for 'frequency' are one of from 'd' (Days),
         'h' (Hours), 'm' (Minutes) and 's' (Seconds).
        e.g. [{'start':"2018-10-23", 'end':"2020-10-30", 'frequency':'s'},{'start':"2016-08-01", 'end':"2017-07-30", 'frequency':'d'}]
            would indicate generating 2 date columns which will have random date at specified 'frequency' level between
    ...
    **kwargs: acceptable key-words arguments
        if target_as_func=True, then pass the valid kwargs for sklearn's make_classification() or make_regression()
        else if num_X_dist=None, then pass the valid kwargs for sklearn's make_gaussian_quantiles()
        else not required
    """
    ...
    if target_as_func:
        ...
        if not is_classification:
            ...
            if n_classes != len(target_dist['probs']):
                raise ValueError("target_dist['probs'] size must be equals to n_classes!")
            y = categorical_distribution_samples(target_dist, n_samples, generate_random_strings=False)
    else:
    ...
    # if perc_missing value is specified, for each feature we impute NAN for that percent of samples
    if perc_missing > 0:
    ...
    # if perc_zeros value is specified, for each numerical feature we impute 0 for that percent of samples
    if perc_zeros > 0:
    ...
    # if not is_classification:
    # # Todo: for regression - understand and evaluate the current logic of relationship building
    #     str_cols = list(set(df.columns) - set(df.select_dtypes('number')))
    #     for col in str_cols:
    #         cats_ = df[col].unique().tolist()
    #         for cat in cats_:
    #             fil_ = (df[col] == cat)
    #             df.loc[fil_, 'target'] = df.loc[fil_, 'target'] + random.randint(0, len(cats_)) * fil_.astype(int)
    # else:
        ...
        for i, x in enumerate(date_cols):
            ...
            if 'frequency' in x.keys():
                ...
                if freq not in ['d', 'h', 'm', 's']:
                    raise ValueError("Invalid frequency value '{}', should be one of 'd' (Days), 'h' (Hours), "
                                     "'m' (Minutes) or 's' (Seconds).".format(freq))
            else:
                freq = 'd'
            df['date_feat_' + str(i + 1)] = [random_date(start_date, end_date, freq) for i in range(n_samples)]
