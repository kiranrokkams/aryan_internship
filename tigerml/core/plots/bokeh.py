import numpy as np
from bokeh.models import Range1d, LinearAxis, Legend, LegendItem
from bokeh.palettes import Spectral6, Spectral11, Plasma256, Inferno256, linear_palette, diverging_palette
...
def get_pretty_glyph_name(glyph):
    if glyph.glyph.__class__.__name__ == 'VBar':
        ...
        return name.replace('_left_parenthesis_', '(').replace('left_parenthesis_', '(')\
            .replace('_right_parenthesis_', ')').replace('_right_parenthesis', ')')
...
def finalize_axis(plot, element, side, include_current, keep_0=False):
    ...
    if [glyph for glyph in glyphs if glyph.glyph.__class__.__name__ in ['VBar', 'Quad']] or keep_0:
        min_ys.append(0)
        max_ys.append(0)
    if glyphs or include_current:
        # import pdb
        # pdb.set_trace()
        axes[0].axis_label = ', '.join(list(set([get_pretty_glyph_name(glyph) for glyph in glyphs] + current_name)))
        max_y = max(max_ys + [np.nan_to_num(glyph.data_source.data[get_glyph_name(glyph)]).max() for glyph in glyphs]) * 1.01
        min_y = min(min_ys + [np.nan_to_num(glyph.data_source.data[get_glyph_name(glyph)]).min() for glyph in glyphs]) * 0.99
...
def add_to_secondary(plot, element):
    ...
    if 'twiny' not in [t for t in pl.extra_y_ranges]:
        ...
        pl.add_layout(LinearAxis(y_range_name="twiny", axis_label=get_pretty_glyph_name(glyph)), 'right')
    for glyph in glyphs:
...
def y_as_datetimeaxis(plot, element, side):
    ...
    if isinstance(pl.yaxis, list) and len(pl.yaxis) > 1:  # 2nd condition is for scenario with only secondary axis
        if side == 'left':
...
def colorize_boxplot(color_axis_values, plot, element):
    ...
    if len(glyph) == 7:
        ...
        # Typical hv.BoxWhisker will have only 7 glyphs (Circle*1, Segment*4 and VBar*2)

        # matching the actual column name with bokeh's ColumnarDataSource column name
        # Eg: 'IMDB.Rating' becomes 'IMDB_full_stop_Rating' and 'Week Day' becomes 'Week_Day'
        cds_col_names = list(pl.renderers[5].data_source.data.keys())
        color_axis_li = '_'.join(e for e in color_axis if e.isalnum()).split('_')
        # color_axis = ''.join([name if all(val in name for val in color_axis_li) else '' for name in cds_col_names])
        # 'Slot' matches with both 'Slot' and 'Channel Slot'
        match_list = [name for name in cds_col_names if all(val in name for val in color_axis_li)]
        if len(match_list) == 1:
        ...
        # (Plasma256 colors are not visually very distinct, but used when levels are more than 12)
        # level_1_factors = [str(x) for x in np.unique(glyph[5].data_source.data[color_axis]) if x not in [np.NaN, None]]
        level_1_factors = color_axis_values[1]  # levels are to be passed from outside as the ColumnarDataSource
        ...
        if len(level_1_factors) < 7:
            ...
            selected_palette = diverging_palette(Plasma256, Spectral11, len(level_1_factors))
        else:
            selected_palette = diverging_palette(Plasma256, Inferno256, len(level_1_factors))

        color_mapper = CategoricalColorMapper(factors=level_1_factors, palette=selected_palette, start=1, end=2)

        # Adding new glyph(s) on top of existing VBar(s) (boxes) to render them with distinct colors
        # glyph[5] -> top half of every box
        # glyph[6] -> bottom half of every box
        pl.vbar(x=glyph[5].glyph.x, top=glyph[5].glyph.top, bottom=glyph[5].glyph.bottom, width=glyph[5].glyph.width,
                source=glyph[5].data_source, color=dict(field=glyph[5].glyph.x, transform=color_mapper),
                line_color='black', fill_alpha=0.8)
        pl.vbar(x=glyph[6].glyph.x, top=glyph[6].glyph.top, bottom=glyph[6].glyph.bottom, width=glyph[6].glyph.width,
                source=glyph[6].data_source, color=dict(field=glyph[6].glyph.x, transform=color_mapper),
                line_color='black', fill_alpha=0.8, legend_field=color_axis)
    ...
    pl.add_layout(pl.legend[0], 'right')  # adding the legend as a separate plot element to the right
...
def dodge_barplot(x_axis_values, plot, element):
    ...
    if plot_orientation == 'upright':
        ...
        for glyph in glyphs_to_modify:
            glyph.x = {'field': axis_field, 'transform': Dodge(value=dodge_pos, range=pl.x_range)}
            glyph.width = 0.2
            dodge_pos += 0.25
        ...
        dodge_pos = (-1*0.5*len(glyphs_to_modify)*0.25) + 0.25*.5
        for glyph in glyphs_to_modify:
            glyph.y = {'field': axis_field, 'transform': Dodge(value=dodge_pos, range=pl.y_range)}
            glyph.height = 0.2
            dodge_pos += 0.25
