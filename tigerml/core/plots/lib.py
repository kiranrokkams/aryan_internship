...
def get_plotter(data=None, x=None, y=None):
	...
	# meta_data = {}
	if data is None:
		assert x is not None and y is not None, 'Need to pass x and y if data is not passed'
...
def get_bokeh_plot(hv_plot):
	line_plot = False
	if hv_plot.__class__.__name__ == 'Curve' or (hasattr(hv_plot, 'children') and 'Curve' in hv_plot.children):
		line_plot = True
...
def save_plot(plot, name):
	...
	assert extension in SAVE_FORMATS, 'Name should have one of these extensions - {}'.format(SAVE_FORMATS.keys())
...
def get_plot_as(plot, type=PLOT_TYPES.bokeh):
	assert str(plot.__module__).startswith('hvplot') or str(plot.__module__).startswith('holoviews'), \
		'get_plot_as accepts only hvPlot or holoviews plot. Got - {}'.format(plot.__module__)
	assert type in PLOT_TYPES, 'type of output should be one of bokeh, plotly or matplotlib'
...
def autosize_plot(plot):
	import holoviews as hv
	plot_options = plot.opts.get().__dict__['kwargs']
	if isinstance(plot, hv.Bars) or isinstance(plot, hv.BoxWhisker) or isinstance(plot, hv.Box) or isinstance(plot, hv.Violin):
