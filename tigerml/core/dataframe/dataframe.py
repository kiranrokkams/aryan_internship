...
def read_csv(path, backend='pandas', **kwargs):
    if backend == 'pandas':
        try:
            ...
            raise Exception('Dataset too big for memory. Try using backend="dask" or "vaex". {}'.format(e))
    elif backend == 'dask':
...
class DataFrame(BackendMixin):
    ...
    def apply(self, func, axis=0, **kwargs):
        if self.backend == BACKENDS.dask:
            if axis == 0:
                series = pd.Series([func(self._data[col]) for col in self._data.columns], index=list(self._data.columns))
    ...
    def categorize(self, set_order=False):
        if self.backend == BACKENDS.dask:
            ...
            for col in cat_cols:
                if set_order:
                    cat_type = CategoricalDtype(categories=self._data[col].dropna().unique().tolist(), ordered=True)
    ...
    def order_categories(self):
        if self.backend == BACKENDS.dask:
            cat_cols = self._data.dtypes[self._data.dtypes.astype(str) == 'category'].index.values
            if len(cat_cols) > 0:
                for col in cat_cols:
    ...
    def convert_datetimes(self, format_dict=None, **pandas_kwargs):
        if not format_dict:
            _LOGGER.info(
                "TigerML Information: It is recommended to pass a dict, with column names as key and their format in the .csv file"
                    ...
                    try:
                        ...
                        if 'could not convert' in str(float_conversion_error):
                            ...
                            # except (TypeError, ValueError) as parsing_error:
                                ...
                                parse(3) -> TypeError: Parser must be a string or character stream, not int
                                parse(3.0) -> TypeError: Parser must be a string or character stream, not float
                                parse('text') -> ValueError: ('Unknown string format:', 'text')
                                parse('0') -> ValueError: day is out of range for month
                                parse('32:0') -> ValueError: hour must be in 0..23
                                """
                                know_exceptions = ['Unknown string format', 'out of range',
                                                   'does not contain a date', 'must be in']
                                if any(subtext in str(parsing_error) for subtext in know_exceptions):
                                    return 0, None
                                else:
                                    _LOGGER.debug(f'printing timestr before raising parsing_error... {timestr}')
                                    raise parsing_error
                        else:
                            _LOGGER.debug(f'printing timestr before raising float_conversion_error... {timestr}')
            ...
            for col in self.dtypes[self.dtypes.astype(str) == 'object'].index:
                ...
                convert_val, format_val = np.vectorize(possible_datetime_value)(col_data_df[col])
                format_df = pd.DataFrame(np.array([convert_val, format_val]).T, columns=['convert_col', 'format_col'])
                ...
                if col_data_df['convert_col'].sum() > 0.3*len(col_data_df):
                    ...
                    if not col_data_df.empty:
                        ...
                        for format_ in inferred_format_list:
                            try:
                                to_datetime(col_data_df[col], format=format_, errors='raise')
                            except ValueError as e:
                                if '(match)' in str(e):
                                    inferred_format_list = np.delete(inferred_format_list,
                                                                     np.where(inferred_format_list == format_))
                                else:
                                    raise e
                        if len(inferred_format_list) > 1:
                            _LOGGER.info(f"Multiple datetime formats {inferred_format_list} were detected for column {col}.")
                            _LOGGER.info(f"\nTo convert {col} to datetime format run..")
                            _LOGGER.info("'df = df.convert_datetimes(format_dict={'"+col+"' : <correct_format>})")
                        elif len(inferred_format_list) == 1:
                            _LOGGER.info(f"Column '{col}' is formatted with format '{inferred_format_list[0]}'.")
                            self._data[col] = to_datetime(col_data, errors='coerce',
                                                          format=inferred_format_list[0])._data
                            converted_cols += [col]
            if len(selected_cols) == 0:
                _LOGGER.info("None of the columns was converted to have datetime format by TigerML")
                _LOGGER.info("To convert any specific column(s) to datetime format run " 
                             "'df = df.convert_datetimes(format_dict={<col_1>: <correct_format>, "
                             "<col_2>: <correct_format>})'")
            elif len(selected_cols)>0 and len(converted_cols) == 0:
                _LOGGER.info(f"Column(s) {selected_cols} was found to have some datetime values, but not converted to "
                             f"datetime format by TigerML due to some inconsistencies in format.")
                _LOGGER.info("To convert any specific column(s) to datetime format run "
                             "'df = df.convert_datetimes(format_dict={<col_1>: <correct_format>, "
                             "<col_2>: <correct_format>})'")
            elif 0 < len(selected_cols) == len(converted_cols):
                _LOGGER.info(f"Column(s) {selected_cols} was found to have some datetime values, and "
                             f"was converted to datetime format by TigerML")
            else:
                non_converted_cols = [item for item in selected_cols if item not in converted_cols]
                _LOGGER.info(f"Column(s) {selected_cols} were found to have some datetime values, but only column(s)"
                             f"{converted_cols} was converted to datetime format by TigerML "
                             f"due to some format inconsistencies in column(s) {non_converted_cols}.")
                _LOGGER.info("To convert any specific column(s) to datetime format run "
                             "'df = df.convert_datetimes(format_dict={<col_1>: <correct_format>, "
                             "<col_2>: <correct_format>})'")
        else:
            for col in format_dict.keys():
                col_data = self._data[col]
                self._data[col] = to_datetime(col_data, errors= 'coerce', format = format_dict[col],
                                              **pandas_kwargs)._data
        return self
