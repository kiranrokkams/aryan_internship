...
def convert_to_tiger_assets(value):
    from tigerml.core.dataframe import DataFrame, Series, Scalar, BACKENDS
    if hasattr(value, '__module__') and value.__module__.split('.')[0] in BACKENDS.keys():
...
def is_series(data):
    ...
    if module == BACKENDS.pandas:
        return isinstance(data, pd.Series) or isinstance(data, list) or \
...
def is_dask(value):
    return get_module(value) == 'dask' or (get_module(value) == 'tigerml' and value.backend == 'dask')
...
def detigerify(data):
    if hasattr(data, '__module__') and data.__module__.startswith('tigerml.core.dataframe') and not callable(data):
        return data._data
    return data


def is_date(series_data):
    if 'datetime' in str(series_data.dtype) and all(series_data.dt.time.astype(str) == '00:00:00'):
        return True
    return False
