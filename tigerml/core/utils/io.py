...
def get_files_in_dir(dir_path, pattern=None, recursive=False, extensions=('xls', 'xlsx', 'csv', 'tsv')):
    if recursive:
...
def read_files_in_dir(dir_path, pattern=None, recursive=False, extensions=('xls', 'xlsx', 'csv', 'tsv')):
    """
    Function to rowbind all the files(csv/tsv/xlsx/xls) in the directory.
    ...
    Parameters:
        ...
        pattern   : None(default);
                        ...
                        (not considerding path)
        recursive : False(default);
                        if True, recursively find all the matching files
                        in subdirectories
    """
    try:
