...
def woe_info_value(target_series, idv_series, target_counts=None):
    ...
    # AdjustedWOE = ln(((Number of non-events in a group + 0.5) / Number of non-events) /
    #                  ((Number of events in a group + 0.5) / Number of events)))
    # https://www.listendata.com/2015/03/weight-of-evidence-woe-and-information.html
    exhaustive_combinations = list(itertools.product(col_target_counts.idv.unique(), col_target_counts.target.unique()))
    exhaustive_combinations_df = pd.DataFrame(exhaustive_combinations, columns=['idv', 'target'])
    col_target_counts = pd.merge(exhaustive_combinations_df, col_target_counts, how="outer", on=["idv", "target"])
    ...
    col_target_counts["col_target_per"] = col_target_counts["col_target_counts"] / col_target_counts["target_counts"]
    col_target_per = col_target_counts.pivot_table(index="idv", columns="target", values="col_target_per", fill_value=0)
    col_target_per.columns = ["NonEvent", "Event"]
    col_target_per["WoE"] = np.log(col_target_per["NonEvent"] / col_target_per["Event"])
    col_target_per["IV"] = (col_target_per["NonEvent"] - col_target_per["Event"]) * col_target_per["WoE"]
