...
def is_datashaded(plot):
	...
	if callback and hasattr(callback, 'operation') and callback.operation.__name__ == 'dynspread':
		return True
	return False
...
def holomap_input_from_dynamicmap(plot):
	plot_dict = {}
	import itertools
	combinations = sorted(list(set([x for x in itertools.product(*[dim.values for dim in plot.kdims]) if len(set(x)) == len(plot.kdims)])))
...
def get_plot_dict_for_dynamicmap(plot):
	plot_dict = {}
	import itertools
	combinations = sorted(list(set([tuple(set(x)) for x in itertools.product(*[dim.values for dim in plot.kdims]) if
	                                len(set(x)) == len(plot.kdims)])))
	for comb in combinations:
		dict = plot_dict
