...
def dict_depth(input_dict):
    ...
    max_depth = max([dict_depth(value) if isinstance(value, dict) else 0 for value in input_dict.values()])
    return max_depth + 1
...
def cartesian_product(*args, unique=True):
    import itertools
    if unique:
        combinations = sorted(list(set([tuple(set(x)) for x in itertools.product(*args) if len(x) == len(args)])))
    else:
...
def get_x_y_vars(data_columns, x_vars=None, y_vars=None):
    ...
    x_vars : `list` or a `string`
        default : empty list, a list of variables for which we want to plot bivariate plots
    y_vars : list or string
        default : empty list, a list of variables for which we want to plot bivariate plots

    Returns
    -------
    x_vars, y_vars : after checking that it exists in the data_columns and set(x_vars) and set(y_vars) are subsets in data_columns
    req_cols : `list`
    ...
    if len(set(x_vars).intersection(set(data_columns))) == 0:
        raise ValueError(
            "Variables not found in dataset: ", set(x_vars) - set(data_columns),
        )
    elif len(set(y_vars).intersection(set(data_columns))) == 0:
        raise ValueError(
            "Variables not found in dataset: ", set(y_vars) - set(data_columns),
        )
    ...
    def fail_gracefully_with_log(func):
        ...
        def inner(*args, **kwargs):
            try:
                ...
                logger.exception('Cannot compute {}. Error - {}'.format(func.__name__, e))
...
def measure_time(logger=None):
    def measure_time_with_log(func):
        ...
        def inner(*args, **kwargs):
            ...
            logger.info('Time taken to run {} is {} seconds'.format(func.__name__, (end_time-start_time).seconds))
            return result
        return inner
...
def set_logger_config(verbose=0, level_name=True, log_time=False):
    ...
    root_logger.handlers[0].setFormatter(logging.Formatter(fmt=logging_formats[level_name][log_time],
                                                           datefmt='%d-%m-%Y %H:%M:%S'))
    ...
    root_logger.handlers[1].handlers[0].setFormatter(logging.Formatter(fmt=logging_formats[level_name][log_time],
                                                                       datefmt='%d-%m-%Y %H:%M:%S'))

    bokeh_logger.handlers[0].setFormatter(logging.Formatter(fmt=logging_formats[level_name][log_time],
                                                                datefmt='%d-%m-%Y %H:%M:%S'))
    ...
    param_logger.handlers[0].setFormatter(logging.Formatter(fmt=logging_formats[level_name][log_time],
                                                            datefmt='%d-%m-%Y %H:%M:%S'))


def set_logger(logger_name, verbose=2, log_dir=None):
    # Todo: enhancement for logging control in terms of decision on showing what/when to console,
    #  by default log level for StreamHandler will be used as 'WARNING' (verbose=2).
    levels = [logging.CRITICAL, logging.ERROR, logging.WARNING, logging.INFO, logging.DEBUG]
    verbose = 0 if verbose < 0 else verbose
    log_level = levels[min(len(levels) - 1, verbose)]
    log_format = {'default': "%(asctime)s - %(levelname)s - %(name)s::%(funcName)s::%(lineno)d - %(message)s",
    ...
    console_handler.setFormatter(logging.Formatter(log_format['default'], datefmt='%Y-%m-%d %H:%M:%S'))
    ...
    if log_dir is not None:
        ...
        info_file_handler = logging.handlers.TimedRotatingFileHandler(log_dir+"/info.log", when="midnight")
        info_file_handler.setLevel(logging.INFO)
        info_file_handler.setFormatter(logging.Formatter(log_format['default'], datefmt='%d-%m-%Y %H:%M:%S'))
        root_logger.addHandler(info_file_handler)
        # error logs file handler
        error_file_handler = logging.handlers.TimedRotatingFileHandler(log_dir+"/error.log", when="midnight")
        error_file_handler.setLevel(logging.ERROR)
        error_file_handler.setFormatter(logging.Formatter(log_format['default'], datefmt='%d-%m-%Y %H:%M:%S'))
