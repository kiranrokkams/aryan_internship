...
def is_dask_scalar(value):
    if hasattr(value, '__module__') and value.__module__.startswith('dask') and value.__class__.__name__ == 'Scalar':
        return True
    return False


def compute_if_dask(value):
    from tigerml.core.dataframe import DataFrame, Series, Scalar
