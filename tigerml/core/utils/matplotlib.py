...
def save_figures_as_html(figures, file_path):
    ...
    if type(figures).__name__ in ["list", "Series"]:
        ...
        for index, fig in enumerate(figures):
            ...
            try:
                plotly_fig = tls.mpl_to_plotly(
                    fig, resize=True, strip_style=True
                )
            except Exception as e:
                print(
                    "Could not convert to plot at index {} to plotly figure.",
        ...
        type(figures).__module__ == "matplotlib.figure" and
        type(figures).__name__ == "Figure"
    ):
