...
def _get_filtered(types):
	filtered_primitives = [prim for prim in all_primitives if any([any([type in input_type.__name__.lower()
	                                                                    for type in types])
	                                                           for input_type in flatten_list(prim.input_types)])]
	return filtered_primitives


datetime_transformers = import_and_load(_get_filtered(['datetime']) + general_primitives)
...
categorical_transformers = import_and_load(_get_filtered(['categorical', 'discrete']) + general_primitives)
