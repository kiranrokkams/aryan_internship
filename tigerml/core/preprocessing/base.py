...
from tigerml.core.utils.pandas import (
    get_bool_cols, get_cat_cols, get_dt_cols,
    get_non_num_cols, get_num_cols
)
...
class DataProcessor:
    ...
    def __init__(self, data, segment_by=None, y=None, y_continuous=None):
        ...
        if segment_by:
            self.segment_by = segment_by if isinstance(segment_by, list) else [segment_by]
        ...
        # self.imputer = Imputer(self.data)
        # self.encoder = Encoder(self.data)
        self.processing_summary = {}
        self.cleaning_summary = {}
        if y:
            #assert isinstance(y, str) # Sanjay: multiple targets can be allowed as we had before??
    ...
    # def _is_discrete(self, series_data):
    #     # from tigerml.core.utils.pandas import is_discrete
    #     from sklearn.utils.multiclass import type_of_target
    #     from tigerml.core.dataframe import convert_to_tiger_assets
    #     val = convert_to_tiger_assets(type_of_target(series_data[series_data.notnull()]))
    ...
    def _check_nulls_in_y_cols(self, y_cols):
        for y in y_cols:
            if self.data[y].isnull().sum() > 0:
                _LOGGER.warning("Dependent variable '{}' having {} null values and the same will be treated in the "
                                "subsequent step of EDA process.".format(y, self.data[y].isnull().sum()))
    ...
    def _set_y_cols(self, y=None):
        ...
        if y:
            ...
            if self.get_non_numeric_columns(self.data[y]):
                _LOGGER.exception('Dependent variable cannot be non numeric. Please process the data.')
                raise Exception('Dependent variable cannot be non numeric. Please process the data.')
            return y
        else:
            return []
    ...
    def _set_current_y(self, y, y_continuous=None):
        ...
        if y_continuous:
            ...
            if len(self.y_cols) > 1:
                _LOGGER.warning('Multiple dependant variables are defined. y_continuous is not passed. Inferring as {}'.
                      format('discrete' if self.is_classification else 'continuous'))
    ...
    def _impute_na(self, df=None, impute_num_na=None, impute_cat_na=None, segment=True):
        ...
        if self.segment_by and segment:
            ...
            for segment in self.all_segments:
                current_imputed = self._impute_na(df[self._get_segment_filter(segment)],
                                                  impute_num_na, impute_cat_na, segment=False)
        ...
        if df.isna().sum().sum() > 0:
            ...
            imputation_summary = imputation_summary[imputation_summary["no_of_missing"]!=0]
    ...
    def _handle_outliers(self, method="mean", lb=None, ub=None, drop=False, cols=None):
        ...
        if not cols:
            ...
            cols = [col for col in cols if col in get_num_cols(df) and col not in get_bool_cols(df)]
    ...
    def _encode_non_numeric(self, y=None, segment=None):
        ...
        if self.segment_by and not segment and y is not None:
            ...
            # return data.groupby(segment_by).apply(lambda df: self._encode_non_numeric(y=y, segment=
            # df[segment_by].iloc[0].values.tolist())).reset_index()
        if segment:
            data = data[self._get_segment_filter(segment)]
        self.encoding_summary = {}
        if self.get_bool_columns():
            _LOGGER.info(
                "Have {} boolean variables. Converting False to 0 and True to 1".format(
                    len(self.get_bool_columns())
                )
            )
            for col in self.get_bool_columns():
                data[col] = data[col] * 1
                self.encoding_summary.update({col: {'original_type': 'bool', 'new_columns': col,
                                                    'method': 'True = 1, False = 0'}})
        if not self.get_cat_columns():
            _LOGGER.info('No categorical variables in the data')
            return data
        if (y is None and self.is_encoded) or (y is not None and y == self._y_for_encoding):
            return data
        # if y is None:
        #     y = self._current_y
        ...
        if encoder.encoding_mapping:
            if segment:
                pass
                # if 'encoded_mappings' not in self.processing_summary:
                #     self.processing_summary['encoded_mappings'] = {}
                # self.processing_summary['encoded_mappings'][', '.join(segment)] = encoder.encoding_mapping
            else:
    ...
    def _clean_data(
        ...
        df = self._handle_outliers(
            method=outlier_method, drop=drop_outliers,
            lb=outlier_lb_cols_dict, ub=outlier_ub_cols_dict,
            cols=outlier_cols
        )
        no_of_rows = len(df)
        if drop_na:
    ...
    def _apply_func(self, func, cols=None, axis=1, segment=True):
        ...
        if cols:
            df = df[cols]
        # assert func.__module__.startswith('tigerml'), 'This function currently only supports tigerml functions'
        if self.segment_by and segment:
            final_data = td.DataFrame()
            from tigerml.core.dataframe.helpers import is_series
            for segment in self.all_segments:
                current_computed = df[self._get_segment_filter(segment)].apply(func, axis=axis)
            ...
            # final_data.index = pd.MultiIndex.from_product([final_data.index, self.all_segments])
            if not final_data.index.name:
                final_data.index.name = 'tigerml_current_index'
            final_data = final_data.reset_index().set_index(self.segment_by + [final_data.index.name])
    ...
    def _filter_cols_by_corr(self, threshold=0.25):
        ...
        for y in self.y_cols:
            ...
            mergeable_corrs = corrs.drop([SUMMARY_KEY_MAP.abs_corr_coef, SUMMARY_KEY_MAP.variable_2], axis=1) \
                .rename(columns={SUMMARY_KEY_MAP.corr_coef: 'Correlation with {}'.format(y)})
            if corr_values is not None:
                corr_values = corr_values.merge(mergeable_corrs, on=SUMMARY_KEY_MAP.variable_1)
            else:
        ...
        corr_values = corr_values.rename(columns={SUMMARY_KEY_MAP.variable_1: 'Variable Name'})
    ...
    def _prepare_data(self, corr_threshold):
        ...
        # pdb.set_trace()
        prep_summary = {}
        constant_cols = [col for col in self.original_columns if self.data[col].nunique() == 1]
        drop_cols = []
        elim_summary = {}
        ...
        if constant_cols:
            ...
            elim_summary['summary'] = [
                'Dropped {} columns - {} having a constant value'.format(len(drop_cols), drop_cols)]
            _LOGGER.info('Dropped {} columns - {} having a constant value'.format(len(drop_cols), drop_cols))

        if self.y_cols and corr_threshold is not None:
            imp_cols, corr_values = self._filter_cols_by_corr(corr_threshold)
            _LOGGER.info('Selected {} features having correlation more than {} with {} column(s)'
                  .format(len(imp_cols), corr_threshold, self.y_cols))
            new_drop_cols = list(set(self.get_numeric_columns()) - set(drop_cols) - set(imp_cols))

            if new_drop_cols:
                drop_cols += new_drop_cols
                message = ['Dropped {} features having correlation less than {} with {} column(s)'.
                               format(len(new_drop_cols), corr_threshold, self.y_cols)]
                _LOGGER.info('Dropped {} features having correlation less than {} with {} column(s)'.
                               format(len(new_drop_cols), corr_threshold, self.y_cols))
                ...
                elim_summary['selected_features'] = corr_values[
                    corr_values['Variable Name'].isin(imp_cols)].reset_index(drop=True)
...
def prep_data(
    ...
    outlier_method: str
        outlier method, one of from 'mean', 'median', 'percentile' and 'threshold'
    drop_outliers:bool
        whether to drop records having outlier values
    outlier_lb_cols_dict: dict
        lower bound outlier limit, If not None, pass a dictionary of columns with custom lower limits for each
    outlier_ub_cols_dict: dict
        upper bound outlier limit, If not None, pass a dictionary of columns with custom upper limits for each
    outlier_cols: list
        column names list to be considered for outlier analysis
    remove_na: bool
        whether to drop records having null values
    impute_num_na: str
        imputation method for numerical columns, one of from "mean", "median", "mode", "constant" and "regression"
    impute_cat_na: str
        imputation method for categorical columns, one of from "mean", "median", "mode", "constant" and "regression"
    stratify: bool
        whether split the data in a stratified fashion, using dv_name as the class labels.
