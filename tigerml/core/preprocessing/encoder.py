...
import pandas as pd
from tigerml.core.utils import compute_if_dask, DictObject
...
class Encoder:
    ...
    def add_encode_rule(self, cols, method, **kwargs):
        cols_already_applied = [col for col in cols if any([col in rule[0] for rule in self.encoding_rules])]
        if cols_already_applied:
            raise Exception('Encoding rule already applied for {}'.format(cols_already_applied))
        if [col for col in cols if col not in self.data]:
            raise Exception('Columns not present in data - {}'.format([col for col in cols if col not in self.data]))
        if method not in self.METHODS:
            raise Exception('Supported imputation methods: {}'.format(self.METHODS.keys()))
    ...
    def _default_encoding(self, encode_columns):
        _LOGGER.info('Encoding categorical variables with default settings which will not be ideal. '
        ...
        if self.y is not None:
            self.encoding_rules.append({'cols': encode_columns, 'method': self.METHODS.target})
            for col in encode_columns:
                min_samples = round(self.MIN_SAMPLE_PERC * len(self.data)) if self.MIN_SAMPLE_PERC else self.MIN_SAMPLE_ABS
                # import pdb
                # pdb.set_trace()
                levels_with_less_min_for_target = [x for x in compute_if_dask(self.data[col].unique())
                                                   if len(self.data[self.data[col] == x]) < min_samples]
                if levels_with_less_min_for_target:
                    _LOGGER.info('{} has levels with less than {}{} values. Target encoding in such cases is not recommended.'
                            .format(col, min_samples, f' ({self.MIN_SAMPLE_PERC*100}%)' if self.MIN_SAMPLE_PERC else ''))
        else:
            for col in encode_columns:
                num_of_levels = self.data[col].nunique()
                if num_of_levels <= self.MAX_BUCKETS:
                    self.encoding_rules.append({'cols': col, 'method': self.METHODS.onehot})
                else:
                    min_samples = (self.MIN_SAMPLE_PERC * len(self.data)) if self.MIN_SAMPLE_PERC else self.MIN_SAMPLE_ABS
                    buckets_with_min_samples = [x for x in compute_if_dask(self.data[col].unique()) if len(self.data[self.data[col]==x]) >= min_samples]
                    # import pdb
                    # pdb.set_trace()
                    if len(buckets_with_min_samples) > num_of_levels * self.LONG_TAIL_CHECK:
                        groups = buckets_with_min_samples
                        self.encoding_rules.append({'cols': col, 'method': self.METHODS.onehot, 'groups': groups})
                    else:
                        _LOGGER.info('CANNOT ENCODE {}. A good encoding method is not found.'.format(col))
                        continue
    ...
    def transform(self, cols=[]):
        if not cols:
            ...
            cols_set = [col for col in cols if not(any(['encoded_{}'.format(col) in data_col for data_col in
                                                            self.data.columns]))]
            if len(cols_set) < len(cols):
                _LOGGER.info('Encoding {} columns. Columns - {} are already encoded.'.format(len(cols_set), list(set(cols) - set(cols_set))))
        else:
            cols_set = cols
        self.encoded_data = self.data
            ...
            for col in cols:
                if method == self.METHODS.onehot:
                    ...
                    if encoded.name in self.encoded_data:
                        _LOGGER.info('{} already exists in data. Overriding it.'.format(encoded.name))
                    ...
                    if encoded.name in self.encoded_data.columns:
                        ...
                        self.encoded_data = td.concat([self.encoded_data, encoded], axis=1)
                self.encoding_summary.update({col: {'original_type': self.data.dtypes.astype(str)[col],
                                                     'new_columns': encoded.columns if hasattr(encoded, 'columns') else encoded.name,
    ...
    def get_encoding_method(self, col_name):
        if [rule for rule in self.encoding_rules if col_name in rule['cols']]:
            return [rule for rule in self.encoding_rules if col_name in rule['cols']][0]['method']
        else:
            return None
    ...
    def onehotEncode(feature, prefix="onehot_encoded", **kwargs):
        ...
        __________
          ...
          prefix : str
            Default is 'OHE'. The prefex will be appended to encoded variable. Ex: 'OHE_VariableName_FactorName'
        ...
        dummies = dummies.rename(columns=dict(zip(list(dummies.columns), [(prefix + '_' + str(x)) for x in dummies.columns])))
        if 'groups' in kwargs:
    ...
    def ordinalEncode(feature, mapper, prefix="ordinal_encoded"):
        ...
        __________
          ...
          mapper : dict
            ...
            Ex: If the variable has following levels low, medium and high and you want to ordinal encode them
            use the following mapper.
            mapper = {'low':1, 'medium':2, 'high':3}
          prefix : str
            Default is 'ORB'. The prefex will be appended to encoded variable. Ex: 'ORB_VariableName_FactorName'
        Returns
        _______
          dataframe :
            Modified dataframe will be returned.
        ...
        if encoded.isnull().sum() > 0:
            _LOGGER.info("Few levels are missing in the mapper, appended such records with nans")
        return encoded, mapper

    @staticmethod
    def targetEncode(feature, target, min_samples=1, smoothing=1, prefix="target_encoded"):
        """
        This transformation is applied on categorical variable for a regression task.
        Each factor value is replaced by the average of the response variable within the factor group.
        ...
        __________
          ...
          smoothing : int
            Default is 1. Smoothens variation in the transformation by giving more weight to the prior average.
          prefix : str
            Default is 'TGE'. The prefex will be appended to encoded variable. Ex: 'TGE_VariableName_FactorName'
        Returns
        _______
          dataframe :
            Modified dataframe will be returned.
        ...
        encoded = td.merge(
            ...
            td.DataFrame(averages.reset_index().rename(columns={'index': feature.name, target.name: 'average'})),
            on=feature.name,
