...
from tigerml.core.utils import DictObject, flatten_list, get_cat_cols, get_num_cols
...
class Imputer(_BaseImputer, TransformerMixin):
    ...
    imputation_summary: dict
        imputations summary with no of missing and method used for imputation for each column
    ...
    METHODS = DictObject({MEAN: "mean", MEDIAN: "median", MODE: "mode", CONSTANT: "constant", REGRESSION: "regression"})
    ...
    def __init__(
        ...
        if num_impute_method not in self.METHODS or cat_impute_method not in self.METHODS:
    ...
    def add_imputation_rule(self, cols, impute_method, constant_value="missing", ind_cols=[], ind_impute_method=None):
        cols_already_applied = [col for col in cols if any([col in rule["cols"] for rule in self.imputation_rules_])]
        if cols_already_applied:
            raise Exception("Imputation rule already applied for {}".format(cols_already_applied))
        if [col for col in cols if col not in self.data_]:
            raise Exception("Columns not present in data - {}".format([col for col in cols if col not in self.data_]))
    ...
    def fit(self, data, y=None):
        ...
        check_array(data, accept_large_sparse=False, dtype=object, force_all_finite="allow-nan", copy = True)
        ...
        for col in self.imputed_data_.columns:
            if compute_if_dask(self.imputed_data_[col].isna().sum() == len(self.imputed_data_)):
                self.imputed_data_ = self.imputed_data_.drop(col, axis=1)
                self.drop_cols_.append(col)
                _LOGGER.info("All the values of {} are missing. Dropping the column.".format(col))
                self.imputation_summary_.update({col: {"no_of_missing": "All values", "method": "Dropped the column"}})
            ...
            for method in self.imputation_rules_:
                if method["impute_method"] == "regression":
                    ...
                    for col in method["cols"]:
                        self.imputer_[col] = self.impute_by_regression(self.data_[col], df, impute_method)
                else:
                    impute_method = method["impute_method"]
                    cols = method["cols"]
                    if all([col in self.num_cols for col in cols]):
                        for col in cols:
                            self.imputer_[col] = self.impute_num(self.data_[col], impute_method=impute_method)
                    elif all([col in self.cat_cols for col in cols]):
                        for col in cols:
                            self.imputer_[col] = self.impute_cat(
                                self.data_[col], impute_method=impute_method, missing_str=method["constant_value"]
                            )
                        ...
                        raise Exception(
                            "numeric and categorical columns cannot be imputed together with {}".format(impute_method)
                        )

        imputed_cols += list(set(flatten_list([rule["cols"] for rule in self.imputation_rules_])))
        for col in set(self.num_cols) - set(imputed_cols):
            ...
            self.imputer_[col] = self.impute_num(self.data_[col], impute_method=self.num_impute_method)
        for col in set(self.cat_cols) - set(imputed_cols):
            ...
            self.imputer_[col] = self.impute_cat(
                self.data_[col], impute_method=self.cat_impute_method, missing_str=self.constant_value
            )
        ...
        if len(non_imputed_cols) > 0:
            for col in non_imputed_cols:
                ...
                _LOGGER.info("Column '{}' is skipped for imputation as its not recognised as either numerical or categorical "
                      "data type.".format(col))
                self.imputation_summary_.update({col: {"no_of_missing": no_of_missing, "imputed with": "Not Imputed"}})
        return self
    ...
    def transform(self, data):
        ...
        check_array(data, accept_large_sparse=False, dtype=object, force_all_finite="allow-nan")
        data = _check_X(data, cols)
        ...
        for col in cols:
            if col not in self.drop_cols_:
                ...
                impute_method = [rule for rule in self.imputation_rules_ if col in rule["cols"]][0]["impute_method"]
                imputed_data_[col] = self.imputer_[col].transform(data_[col].to_numpy().reshape(-1, 1))
                self.imputation_summary_.update({col: {"no_of_missing": no_of_missing, "imputed with": impute_method}})
    ...
    def get_imputation_method(self, col_name):
        if [rule for rule in self.imputation_rules_ if col_name in rule["cols"]]:
            return [rule for rule in self.imputation_rules_ if col_name in rule["cols"]][0]["impute_method"]
        else:
            return None
    ...
    def impute_num(feature, impute_method="mean", constant_value=None):
        ...
        if impute_method == "median":
            ...
            raise Exception('Supported values for "impute_method" are - mean, median, mode')
        return imp
    ...
    def impute_cat(feature, impute_method="constant", missing_str="missing"):
        ...
        ), "feature is not Categorical, imputeMissingValCat can impute only Categorical variables"
    ...
    def impute_by_regression(target, df, impute_method="mean"):
        """
        This method will impute the missing values of the target variable by fitting linear/logistic equation.
        Unlike MICE, this method will create single equation for the target variable.
        ...
        __________
          ...
          impute_method : str
            Default is 'Mean'. Strategy on how to impute missing values in independent variables of the equation.
