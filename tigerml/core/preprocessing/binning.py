...
def supervised_binning(feature, target, prefix="SPE"):
    """
    This method finds optimal bins for the given feature such that the information gain is maximized w.r.t to target
    variable. This transformation can be used only for classification task.
...
def binning(series, n=None, size_of_bin=None, labels=None, quantile=None, value=None, size=None, left=True, target=None):
    """
    ...
    try:
        if n:
            ...
            assert not np.issubdtype(target.dtype, np.number), "target is a number and not Categorical, " \
                                                               "supervised binning can only be applied for classification tasks"
            assert len(series) == len(target), "input series and target should be of the same length"
