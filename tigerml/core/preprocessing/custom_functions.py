...
FUNCTIONS_DICT = {
	'EQUAL': {
		'no_of_params': 2,
		'input_type': ['IntergerField', 'DecimalField', 'TextField', 'DateField', 'DateTimeField', 'TimeField'],
...
def CONTAINS(find_in, find_this):
	if type(find_in).__name__ == 'Series':
		...
		if type(find_this).__name__ == 'Series':
			...
			return (str(find_this) if type(find_in).__name__ != 'list' else find_this) in \
				   (str(find_in) if type(find_in).__name__ != 'list' else find_in)
	values = [(str(x[0]) if type(x[1]).__name__ != 'list' else x[0]) in
			  (str(x[1]) if type(x[1]).__name__ != 'list' else x[1]) for x in zip(list(find_this), list(find_in))]
...
def UNIQUECOUNT(*args):
	...
	for arg in [x for x in args if type(x).__name__ == 'Series']:
		if type(arg.iloc[0]).__name__ == 'list':
			...
			merged_args = merged_args + arg
	merged_args = merged_args + list(set(flatten_list(*[x for x in args if type(x).__name__ != 'Series'])))
...
def SHIFT(shift, values, sort=None):
	# import pdb
	# pdb.set_trace()
	# if get_val.name not in df:
	# 	if get_val.index.name:
	# 		df = df.merge(get_val, how='left', left_on=get_val.index.name, right_index=True)
	# 	else:
	# 		df = df.merge(get_val, how='left', left_index=True, right_index=True)
	# shifted_df = df.shift(periods=shift)
	# return shifted_df[get_val.name]
	if sort:
...
def GROUP(df, string_dict, group_by, agg_expression):
	...
	return df.groupby(by=group_by.name).apply(lambda s: evaluate_expression_old(agg_expression, s, string_dict))
	# return pd.concat([df, group_by.rename('group_by')], axis=1).groupby(by='group_by')\
	# 	.apply(lambda df: evaluate_expression(agg_expression, df))
...
def IF(data, string_dict, check, func, else_func):
	...
	if type(check).__name__ == 'Series':
		...
		new_df['values'] = None
		new_df.loc[new_df['check'], 'values'] = new_df[new_df['check']].apply(lambda x: evaluate_expression_old(func, x, string_dict), axis=1)
		new_df.loc[~new_df['check'], 'values'] = new_df[~new_df['check']].apply(lambda x: evaluate_expression_old(else_func, x, string_dict), axis=1)
...
def ADDYEARS(datetime, no_of_years):
	if type(datetime).__name__ == 'Series':
		if type(no_of_years).__name__ == 'Series':
			return pd.concat([datetime, no_of_years], axis=1).apply(lambda s: ADDYEARS(s[0], s[1]), axis=1)
		else:
...
def ADDMONTHS(datetime, no_of_months):
	if type(datetime).__name__ == 'Series':
		if type(no_of_months).__name__ == 'Series':
			return pd.concat([datetime, no_of_months], axis=1).apply(lambda s: ADDMONTHS(s[0], s[1]), axis=1)
		else:
...
def ADDDAYS(datetime, no_of_days):
	if type(datetime).__name__ == 'Series':
		if type(no_of_days).__name__ == 'Series':
			return pd.concat([datetime, no_of_days], axis=1).apply(lambda s: ADDDAYS(s[0], s[1]), axis=1)
		else:
...
def ADDHOURS(datetime, no_of_hours):
	if type(datetime).__name__ == 'Series':
		if type(no_of_hours).__name__ == 'Series':
			return pd.concat([datetime, no_of_hours], axis=1).apply(lambda s: ADDHOURS(s[0], s[1]), axis=1)
		else:
...
def ADDMINUTES(datetime, no_of_minutes):
	if type(datetime).__name__ == 'Series':
		if type(no_of_minutes).__name__ == 'Series':
			return pd.concat([datetime, no_of_minutes], axis=1).apply(lambda s: ADDMINUTES(s[0], s[1]), axis=1)
		else:
...
def ADDSECONDS(datetime, no_of_seconds):
	if type(datetime).__name__ == 'Series':
		if type(no_of_seconds).__name__ == 'Series':
			return pd.concat([datetime, no_of_seconds], axis=1).apply(lambda s: ADDSECONDS(s[0], s[1]), axis=1)
		else:
...
def DATE(year, month=1, day=1):
	...
	if 'Series' in [type(year).__name__, type(month).__name__, type(day).__name__]:
		...
		year = year.astype(int) if type(year).__name__ == 'Series' else pd.Series([int(year)] * length)
		month = month.astype(int) if type(month).__name__ == 'Series' else pd.Series([int(month)] * length)
		day = day.astype(int) if type(day).__name__ == 'Series' else pd.Series([int(day)] * length)
		return pd.to_datetime(pd.concat([year.rename('year'), month.rename('month'), day.rename('day')], axis=1)).dt.date
...
def TIME(hours, minutes=0, seconds=0):
	...
	if 'Series' in [type(hours).__name__, type(minutes).__name__, type(seconds).__name__]:
		length = len([x for x in [hours, minutes, seconds] if type(x).__name__ == 'Series'][0])
		year = pd.Series([int(2000)] * length)
		month = pd.Series([int(1)] * length)
		day = pd.Series([int(1)] * length)
		hours = hours.astype(int) if type(hours).__name__ == 'Series' else pd.Series([int(hours)] * length)
		minutes = minutes.astype(int) if type(minutes).__name__ == 'Series' else pd.Series([int(minutes)] * length)
		seconds = seconds.astype(int) if type(seconds).__name__ == 'Series' else pd.Series([int(seconds)] * length)
		return pd.to_datetime(pd.concat([year.rename('year'), month.rename('month'), day.rename('day'),
										 hours.rename('hours'), minutes.rename('minutes'), seconds.rename('seconds')], axis=1)).dt.time
...
def DATETIME(year, month=1, day=1, hours=0, minutes=0, seconds=0, explicit_params=None):
		...
		if type(year.iloc[0]).__name__ == 'date':
			if type(month).__name__ == 'Series':
				...
				if type(month.iloc[0]).__name__ == 'time':
					...
					seconds = hours
					return pd.to_datetime(year.astype(str) + ' ' + TO_STRING(TIME(hours, minutes, seconds)))
	if 'Series' in [type(year).__name__, type(month).__name__, type(day).__name__, type(hours).__name__,
					type(minutes).__name__, type(seconds).__name__]:
		return pd.to_datetime(TO_STRING(DATE(year, month, day)) + ' ' + TO_STRING(TIME(hours, minutes, seconds)))
	...
	try:
		return datetime(year=int(year), month=int(month), day=int(day), hour=int(hours), minute=int(minutes),
						second=int(seconds))
	except ValueError:
		return None
...
def RANGE(start, increment, end=None, no_of_bins=None):
	...
	if data_type:
		...
		output = [time(hour=int(element/60), minute=int(element%60), second=int((element%1)*60)) for element in output]
	return output
