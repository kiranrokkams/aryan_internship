...
def get_component_in_format(component, format):
	...
	# child_class = import_from_module_path('tigerml.core.reports.{}.{}'.format(module, child_class_name))
...
def create_components(contents, flatten=False, format='html'):
	...
	for content in enforce_iterable(contents):
		...
		if isinstance(content, str):
		...
		elif isinstance(content, CLASSES[CG_CLASS]) or isinstance(content, CLASSES[COMPONENT_CLASS]):
			component = content
			...
			component = CLASSES[TABLE_CLASS](compute_if_dask(content)._data, title=content_name)
			...
			if flatten:
				...
				component = (content_name, create_components(content, flatten=False, format=format))
				if format == 'html':
