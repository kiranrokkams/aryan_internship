...
def percentage_format(s):
    return "number-format: 0.00%".format(s)


def time_format(s):
    return "number-format: hh:mm:ss.000".format(s)


def date_format(s):
    return "number-format: yyyy:mm:dd".format(s)


def datetime_format(s):
    return "number-format: yyyy:mm:dd hh:mm:ss.000".format(s)
...
def get_max_width(col, styler=None):
    ...
    if len(col) > 0:
        if styler is not None and col.name is not None:
            ...
            cell_values = [cell['display_value'] for cell in flatten_list(styler._translate()['body']) if 'col{}'.format(index) in cell['id']]
    ...
    if col.name:
        if isinstance(col.name, tuple) and styler is not None:
            ...
            for level in range(0, levels):
                no_of_cols_under_level = len([x for x in styler.data.columns if col.name[:level+1] == x[:level+1]])
                max_width = max(max_width, len(col.name[level])*0.9/no_of_cols_under_level)
            return max_width
        else:
