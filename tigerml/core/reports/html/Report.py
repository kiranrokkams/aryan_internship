...
from .helpers import title_html, has_single_component
from tigerml.core.utils import prettify_slug, slugify, get_extension_of_path
...
class HTMLReport:
	...
	def save(self, path='', needs_folder=True):
		...
		template_file = open(os.path.join(HERE, 'report_resources', 'template.html'), "r")
		html_content = template_file.read()
		template_file.close()
		style_file = open(os.path.join(HERE, 'report_resources', 'style.css'), "r")
		custom_style = style_file.read()
		style_file.close()
		filesaver_script_file = open(os.path.join(HERE, 'report_resources', 'FileSaver.min.js'), "r")
		...
		html_content = html_content.replace('{{report_title}}', prettify_slug(self.name))\
			.replace('{{report_head}}', INLINE.render()).replace('{{custom_style}}', custom_style)\
			.replace('{{custom_script}}', custom_script).replace('{{file_name}}', file_name)\
			.replace('{{report_nav}}', self._get_nav()).replace('{{resource_path}}', resource_path)\
...
class HTMLDashboard:
	...
	def insert(self, content, rel_position='below', rel_component_id=None):
		...
		elif [c for c in self.components[:-1] if getattr(c, 'on_{}'.format(rel_position)) is None]:
			rel_component = [c for c in self.components[:-1] if getattr(c, 'on_{}'.format(rel_position)) is None][-1]
		else:
			rel_component = None
		if rel_component:
			if rel_position in ['left', 'above']:
				if getattr(rel_component, 'on_{}'.format(rel_position)):
					existing_component = getattr(rel_component, 'on_{}'.format(rel_position))
					setattr(content, 'on_{}'.format(rel_position), existing_component)
				setattr(rel_component, 'on_{}'.format(rel_position), component)
			else:
				if getattr(rel_component, 'on_{}'.format(rel_position)):
					existing_component = getattr(rel_component, 'on_{}'.format(rel_position))
					setattr(existing_component, 'on_{}'.format(opp_direction[rel_position]), component)
				setattr(component, 'on_{}'.format(opp_direction[rel_position]), rel_component)
	...
	def _get_nav(self):
		link_str = '<a id="nav_to_{}" class="closed">{}</a>'.format(slugify(self.name), prettify_slug(self.name))
		children_links = ''
		if len(self.components) == 1 and has_single_component(self.components[0]):
			links = '<li>{}</li>'.format(link_str.replace('class="closed"', f'href="#{slugify(self.name)}"'))
		else:
			for component in self.components:
				parent_name = None
				if isinstance(component.content, ComponentGroup):
					parent_name = self.name
				children_links += '<li>{}</li>'.format(component._get_link(parent_name=parent_name))
			links = '<li class="group">{}<ul>{}</ul></li>'.format(link_str, children_links)
		return links
	...
	def to_html(self, resource_path):
		...
		html_str = '<div class="dashboard" id="{}">{}</div>'.format(slugify(self.name), html_str)
		return html_str
