from .Report import HTMLReport, HTMLDashboard, HTMLComponent, HTMLComponentGroup
from .contents import HTMLText
from tigerml.core.utils import time_now_readable


def create_html_report(contents, columns=2, save=True, name='', path='', split_sheets=True):
	if not name:
	...
	if split_sheets:
		...
		for content in contents:
			...
			if not isinstance(content, dict):
				content = {'': content}
			report_dict, needs_folder_local = create_html_dashboard(content, name=content_name, columns=columns)
			needs_folder = needs_folder or needs_folder_local
			report.append_dashboard(report_dict)
	else:
		report_dict, needs_folder = create_html_dashboard(contents, name=name, columns=columns)
		report.append_dashboard(report_dict)
	# else:
	# 	report.append_dashboard(create_html_dashboard(contents, name='Sheet1', columns=columns))
	if save:
...
def create_html_dashboard(contents, name='', columns=2, flatten=False):
	...
	# 	content_name = content
	# 	content = contents[content_name]
	cg, needs_folder = create_component_group(contents, dash, columns=columns, flatten=flatten)
	dash.append(cg)
	...
	if [component for component in components if isinstance(component, tuple)]:
		...
		for component in components:
			if isinstance(component, tuple):
				...
				new_cg = group_components(component[1], dashboard, component[0], columns=columns, flatten=flatten)
...
def create_component_group(contents, dashboard, name='', columns=2, flatten=False):
	...
	if isinstance(contents, str):
		...
		components, needs_folder = create_components(contents, flatten=flatten, format='html')
	# if len(contents) == 1:
	# 	columns = 1
	cg = group_components(components, dashboard, name=name, columns=columns, flatten=flatten)
