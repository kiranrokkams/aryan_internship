...
class HTMLComponent(Component):
	...
	def __init__(self, content, dashboard, group=None):
		assert content.__module__.startswith('tigerml.core.reports.html') or content.__class__.__name__ == 'HTML'
	...
	def _get_link(self, parent_name=''):
		# if self.name == 'correlation_table' or self.content.name == 'correlation_table':
		# 	import pdb
		# 	pdb.set_trace()
		# link_name = ('{} '.format(parent_name) if parent_name else '') + self.content.name
		# if not self.name:
		# 	self.name = self.content.name
		# try:
		# 	isinstance(self.content, ComponentGroup) and (
		# 				len(self.content.components) > 1 or self.content.components[0].name)
		# except Exception as e:
		# 	import pdb
		# 	pdb.set_trace()
		if isinstance(self.content, ComponentGroup) and self.content.components and \
				(len(self.content.components) > 1 or self.content.components[0].name):
			if not [x for x in self.content.components if x.content.name]:
				link_str = '<a id="nav_to_{}" href="#{}">{}</a>'.format(slugify(self.content.name),
				                                                        slugify(self.content.name),
				                                                        prettify_slug(self.name))
			else:
				link_str = '<a id="nav_to_{}" class="closed">{}</a>'.format(slugify(self.content.name),
				                                                            prettify_slug(self.name))
				if not self.name:
					return self.content._get_links(parent_name)[4:-5]      # remove the <ul> links
				links_to_children = self.content._get_links()
				link_str = '<li class="group">{}{}</li>'.format(link_str, links_to_children)
		else:
			link_str = '<a id="nav_to_{}" href="#{}">{}</a>'.format(slugify(self.content.name),
			                                                        slugify(self.content.name),
			                                                        prettify_slug(self.name))
	...
	def to_html(self, resource_path):
		...
		content_html = ''
		if self.content.name and self.show_title and not isinstance(self.content, HTMLComponentGroup):
			content_html += title_html(prettify_slug(self.content.name))
		if isinstance(self.content, HTMLImage) or isinstance(self.content, HTMLComponentGroup):
		...
		return '<div class="component" id="{}">{}</div>'.format(slugify(self.content.name), content_html)

	def save(self, name=''):
		html_str = '<html><body>{}</body></html>'.format(self.to_html(resource_path='.'))
		if not name:
			name = self.name
...
class HTMLComponentGroup(ComponentGroup):
	...
	def to_html(self, resource_path):
		html_str = ''
		if len(self.components) == 1 and not isinstance(self.components[0].content, ComponentGroup):
			if self.name and self.components[0].content.name:
				self.name += ' - '
			self.name += self.components[0].content.name if self.components[0].content.name else ''
		...
		return '<div class="component_group cols_{}">{}</div>'.format(str(columns), html_str)
