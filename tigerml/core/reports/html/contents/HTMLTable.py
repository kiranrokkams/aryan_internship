                ...
                if rule['cols']:
                    if rule['col_wise']:
                        ...
                        for col in rule['cols']:
                            self.styler.background_gradient(subset=[col], cmap=rule['style'], axis=0)
                    else:
                        self.styler.background_gradient(subset=rule['cols'], cmap=rule['style'], axis=0)
                elif rule['rows']:
                    self.styler.background_gradient(subset=rule['rows'], cmap=rule['style'], axis=1)
        html_str = '<div class="content_inner">{}</div>'.format(self.styler.render())
        return '<div class="content table_content {}">{}</div>'.format('apply_datatable' if self.datatable else
                                                                       'no_datatable', html_str)
