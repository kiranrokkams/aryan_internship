	...
	def to_html(self, resource_path=''):
		...
		if isinstance(self.plot, str):
			...
			if self.chart_module == 'bokeh':
				...
				plot_html = 'Could not generate. Error - Given plot input is neither bokeh nor plotly.'
		return '<div class="content chart_content"><div class="content_inner">{}</div></div>'.format(plot_html)
