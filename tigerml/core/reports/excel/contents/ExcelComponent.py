...
class ExcelComponent(Component):
	...
	def __init__(self, content, dashboard, group=None):
		# import pdb
		# pdb.set_trace()
		assert content.__module__.startswith('tigerml.core.reports.excel') or content.__module__.startswith('xlsxwriter')
	...
	def save_content(self, worksheet, workbook, top_row, left_col):
		...
		if self.content.__module__.startswith('tigerml.core.reports.excel'):
			# from tigerml.core.reports.excel import ExcelTable
			# if isinstance(self.content, ExcelTable) or isinstance(self.content, ComponentGroup):
			...
			if isinstance(self.content, ExcelText):
				if self.group:
					...
					try:
						width = self.group.components[current_index-self.group.columns].width
					except IndexError:
						try:
							width = self.group.components[current_index+self.group.columns].width
						except IndexError:
					...
					width = self.on_above.width if self.on_above else self.on_below.width if self.on_below else None
				if width:
					width = int(width)
	...
	def save_to(self, worksheet, workbook):
		if self.on_left:
			self.left_col = int(round(self.on_left.left_col + self.on_left.width + self.dashboard.hor_spacing))
			self.top_row = self.on_left.top_row
		if self.on_above:
			self.top_row = int(round(self.on_above.top_row + self.on_above.height + self.dashboard.ver_spacing))
