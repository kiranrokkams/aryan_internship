    ...
    def to_excel(self, workbook, sheet_name="Sheet1", startrow=0, startcol=0):
        ...
        for styler_rule in self.styler_rules:
            if 'map' in styler_rule:
                ...
                styler = self.styler
            styler.applymap(styler_rule['func'], subset=styler_rule['subset'] if 'subset' in styler_rule else None)
        self.styler.to_excel(excel_writer,
            ...
            for format_rule in self.conditional_formatters:
                ...
                if format_rule['cols'] is not None:
                    for col in format_rule['cols']:
                        ...
                        worksheet.conditional_format(row_add, col_index, row_add + len(self.data), col_index,
                ...
                if format_rule['rows'] is not None:
                    for row in format_rule['rows']:
                        ...
                        worksheet.conditional_format(row_index, col_add, row_index, len(self.data.columns) + col_add,
                                                     options=format_rule['options'])
                if format_rule['index'] is not None:
                    ...
                    if format_rule['index'] is True:
                        col_range = range(col_start_index, col_start_index+len(self.data.index.names))
                    elif isinstance(format_rule['index'], list):
                        col_range = [(col_start_index + level) for level in format_rule['index']]
                    else:
                        continue
                    for col_index in col_range:
                        worksheet.conditional_format(0, col_index, len(self.data), col_index,
                                                     options=format_rule['options'])
                if format_rule['header'] is not None:
                    row_start_index = 0 + startrow
                    if format_rule['header'] is True:
                        row_range = range(row_start_index, row_start_index + len(self.data.columns.names))
                    elif isinstance(format_rule['header'], list):
                        row_range = [(row_start_index + level) for level in format_rule['header']]
                    else:
                        continue
                    for row_index in row_range:
                        worksheet.conditional_format(row_index, 0, row_index, len(self.data.columns),
                                                     options=format_rule['options'])
                if format_rule['bool_map'] is not None:
                    ...
                    if format_rule['col_wise']:
                        for col in format_rule['cols']:
                            col_index = list(self.data.columns).index(col) + col_add
                            worksheet.conditional_format(1, col_index, len(self.data), col_index, options=format_rule['options'])
                    else:
                        for row in format_rule['rows']:
                            row_index = list(self.data.index).index(row) + row_add
                            worksheet.conditional_format(row_index, 0, row_index, len(self.data.columns),
        ...
        bool_rules = [rule for rule in self.cell_formatters if rule['bool_map'] is not None]
        ...
        if col_rules:
            # import pdb
            # pdb.set_trace()
            for col in set(flatten_list([x['cols'] for x in col_rules])) & set(list(self.data.columns)):
                ...
                width_val, style = process_col_style(combined_style, self.data[col], self.styler)
                col_index = list(self.data.columns).index(col) + col_add + startcol
                style_format = workbook.add_format(style)
                worksheet.set_column(col_index, col_index, width=width_val, cell_format=style_format,
        ...
        if row_rules:
            ...
            if str(self.data.index.dtype) != 'int64':
                ...
                row_vals = set([x for x in row_vals if not isinstance(x, int)] + int_row_names) & set(self.data.index)
            for row in row_vals:
                all_rules_for_row = [x for x in row_rules if
                                     row in x['rows'] or list(self.data.index).index(row) in x['rows']]
                combined_style = {}
                combined_options = {}
                for each_rule in all_rules_for_row:
                    ...
                    if each_rule['options'] is not None:
                        combined_options.update(each_rule['options'])
                height_val, style = process_row_style(combined_style, self.data.loc[row], self.styler)
                row_index = list(self.data.index).index(row) + row_add + startrow
                style_format = workbook.add_format(style)
                worksheet.set_row(row_index, height=height_val, cell_format=style_format, options=combined_options)
        if self.params['index'] and index_rules:
            # import pdb
            # pdb.set_trace()
            if not isinstance(self.data.index, pd.MultiIndex):
                width_val, style, options = create_index_format(index_rules, self.data.index)
                ...
                for level in range(0, len(self.data.index.names)):
                    all_rules_for_index = [x for x in index_rules if level in x['index'] or x['index'] is True]
                    width_val, style, options = create_index_format(all_rules_for_index,
                                                                    self.data.index.get_level_values(level))
        ...
        if self.params['header'] and header_rules:
            ...
            if not isinstance(self.data.columns, pd.MultiIndex):
                height_val, style, options = create_header_format(header_rules, self.data.columns)
                row_index = 0 + startrow
                style_format = workbook.add_format(style)
                worksheet.set_row(row_index, height=height_val, cell_format=style_format, options=options)
            else:
                for level in range(0, len(self.data.columns.names)):
                    all_rules_for_header = [x for x in header_rules if level in x['header'] or x['header'] is True]
                    height_val, style, options = create_index_format(all_rules_for_header,
