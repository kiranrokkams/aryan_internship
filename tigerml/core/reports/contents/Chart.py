from tigerml.core.utils.plots import holomap_input_from_dynamicmap, get_plot_dict_for_dynamicmap


class Chart:

	def __new__(cls, input, name=''):
		if input.__class__.__name__ == 'DynamicMap' and input.kdims and input.collate().__class__.__name__ != 'DynamicMap':
	...
	def __init__(self, input, name=''):
		...
		if str(input.__module__).startswith('bokeh'):
			self.plot = input
		elif str(input.__module__).startswith('holoviews') or str(input.__module__).startswith('hvplot'):
			if input.__class__.__name__ not in ['DynamicMap', 'HoloMap']:
				from tigerml.core.plots import get_bokeh_plot, autosize_plot
				...
				if input.__class__.__name__ == 'DynamicMap' and input.kdims: # If dimensioned DynamicMap, convert to holomap
					plot_dict = holomap_input_from_dynamicmap(input)
					import holoviews as hv
					input = hv.HoloMap(plot_dict, kdims=[dim.name for dim in input.kdims])
				import holoviews as hv
				bokeh_renderer = hv.renderer('bokeh')
				plot_html = bokeh_renderer.static_html(input)
				head_tag = plot_html[plot_html.index('<head>') + 6:plot_html.index('</head>')]
				script_tags = []
				while '<script' in head_tag:
					tag = head_tag[head_tag.index('<script'):head_tag.index('</script>') + 9]
					script_tags += [tag]
				...
				self.plot = script + html_body[html_body.index('<body>')+6:html_body.index('</body>')]
	...
	def chart_module(self):
		return 'str' if isinstance(self.plot, str) else self.plot.__module__.split('.')[0]
