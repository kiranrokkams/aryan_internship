...
class Image:
	...
	def __init__(self, input, name='', format='png'):
		if isinstance(input, str):
			...
			from tigerml.core.plots import get_mpl_plot, autosize_plot, get_bokeh_plot
			...
			try:
				...
				# self.image_data = Image.open(image_data)
				from bokeh.io.export import get_screenshot_as_png
				self.image_data = get_screenshot_as_png(bokeh_plot)
			except Exception as e:
				print('COULD NOT SAVE PLOT - {}. ERROR - {}'.format(name, e))
				self.image_data = Image.new(mode="RGB", size=(1, 1))
			...
			raise Exception('Unsupported input for Image creation - {}'.format(type(input)))
		if format != 'svg':
			self._width, self._height = self.image_data.size
