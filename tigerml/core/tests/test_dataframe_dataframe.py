...
from hypothesis.strategies import composite, floats, sampled_from, text, data, integers, lists
import pytest
...
def make_date_strings(draw, formatting_type, have_unambiguous_date='no'):
    ori_date = []
    str_date = []
    diff_formats = {1: ["str(day_val)+'-'+str(month_val)+'-'+str(year_val)", "%d-%m-%Y"],
    ...
    ts_df = pd.DataFrame(data={'ori_date': ori_date, 'str_date ' + diff_formats[formatting_type][1]: str_date})
...
@pytest.mark.parametrize("format_type",
                         [1],# 2, 3, 4, 5, 6, 7, 8],
                         ids=id_func)
@pytest.mark.parametrize("unambiguous_date",
                         ['first', 'last'], #'no'],
                         # TODO: Fix the failing test case auto-no unambiguous_date-%d-%m-%Y
                         ids=id_func)
@pytest.mark.parametrize("convert_datetime",
                         ['auto', 'explicit'])
@settings(max_examples=50, deadline=None)
@given(test_df_=data())
def test_convert_datetimes(test_df_, convert_datetime, format_type, unambiguous_date):

    test_df = test_df_.draw(make_date_strings(formatting_type=format_type, have_unambiguous_date=unambiguous_date))
    col_nm = test_df.columns[1]
    tdf = td.DataFrame(test_df.copy())
    if convert_datetime == 'auto':
        tdf = tdf.convert_datetimes()
    else:
        # tdf = tdf.convert_datetimes(format_dict={col_nm: {'format': id_func(format_type)}})
