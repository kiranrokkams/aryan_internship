...
def data(draw):
    ...
    string_options = [
        ...
        *[choice(string.ascii_uppercase) + "  "+choice(string.ascii_lowercase) for i in range(1,3)],
        *[choice(string.ascii_letters) + choice(string.punctuation) for i in range(1, 10)],
    ]
...
def test_string_cleaning(series):
    ...
    regex= "^[^\s]+([\s]{0,1}[A-Za-z0-9" + special_chars_to_keep + "]*)$"
    assert (string_series.str.match(regex)).all()

    string_series = string_cleaning(series, strip=False)
    regex = "^([A-Za-z0-9\s" + special_chars_to_keep + "]*)$"
    assert (string_series.str.match(regex)).all()

    special_chars_to_keep = "_"
    regex = "^[a-z0-9" + special_chars_to_keep + "]+([\s]{0,1}[a-z0-9" + special_chars_to_keep + "]*)$"
    #regex2="^[^\s]+([\s]{0,1}[a-z0-9" + special_chars_to_keep + "]*)$"
