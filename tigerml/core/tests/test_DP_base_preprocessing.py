...
from hypothesis.strategies import composite, floats, sampled_from, lists, tuples, datetimes, data, integers
...
def make_named_data(draw, col):
    df = draw(data_frames(index=range_indexes(min_size=5),
                          columns=[column('Explicit_obj_col_with_nums', elements=sampled_from([5, 5.3]), dtype=object),
                                   column('Explicit_obj_col_with_mix', elements=sampled_from([5, 5.3, 'Text']),
                                          dtype=object),
                                   column('Implicit_nums_col', elements=sampled_from([5, 5.3])),
                                   column('Implicit_mix_col', elements=sampled_from([5, 5.3, 'Text']) | datetimes(
                                       min_value=pd.Timestamp('1/1/1995'), max_value=pd.Timestamp('1/1/2005'))),
                                   column('Implicit_bool_col', elements=sampled_from([True, False])),
                                   column('Explicit_obj_col_with_bool', elements=sampled_from([True, False]),
                                          dtype=object),
                                   column('Implicit_dt_col', datetimes(min_value=pd.Timestamp('1/1/1995'),
                                                                       max_value=pd.Timestamp('1/1/2005'))),
                                   column('Explicit_obj_col_with_dt',
                                          elements=datetimes(min_value=pd.Timestamp('1/1/1995'),
                                                             max_value=pd.Timestamp('31/12/2005')), dtype=object)]))
    return df[col]
...
@pytest.mark.parametrize("selected_col",
                         [['Explicit_obj_col_with_nums'],
                          ['Explicit_obj_col_with_mix'],
                          ['Implicit_nums_col'],
                          ['Implicit_mix_col']],
                         ids=['Explicit_obj_col_with_nums', 'Explicit_obj_col_with_mix', 'Implicit_nums_col',
                              'Implicit_mix_col'])
...
def test_get_numeric_columns(test_df, selected_col):
    ...
    test_df_[new_col_nm]=test_df_.apply(lambda x: True if type(x[old_col_nm]) in [int, float, np.int64, np.float64] else False , axis=1)
    if (len(test_df_.groupby([new_col_nm], sort=False).count()) == 1) & test_df_[new_col_nm][0]==True:
...
@pytest.mark.parametrize("selected_col, exp_output",
                         [(['Implicit_dt_col'], ['Implicit_dt_col']),
                          (['Explicit_obj_col_with_dt'], ['Explicit_obj_col_with_dt']),
                          (['Explicit_obj_col_with_nums'], []),
                          (['Explicit_obj_col_with_mix'], [])],
                         ids=['Implicit_dt_col', 'Explicit_obj_col_with_dt', 'Explicit_obj_col_with_nums',
                              'Explicit_obj_col_with_mix'])
@settings(max_examples=20, deadline=None)
@given(test_df=data())
def test_dt_columns(test_df, selected_col, exp_output):
    ...
    if selected_col[0]=='Explicit_obj_col_with_dt':
        an_obj.data['Explicit_obj_col_with_dt']=an_obj.data['Explicit_obj_col_with_dt'].apply(lambda x: pd._libs.tslib.Timestamp(str(x.year)+'-'+str(x.month)+'-'+str(x.day)+' '+str(x.hour)+':'+str(x.minute)+':'+str(x.second)+'.'+str(x.microsecond)))
...
@pytest.mark.parametrize("selected_col, exp_output",
                         ...
                         ids=['Explicit_obj_col_with_nums', 'Explicit_obj_col_with_mix', 'Implicit_mix_col',
                              'Implicit_nums_col', 'Implicit_bool_col', 'Explicit_obj_col_with_bool',
                              'Implicit_dt_col', 'Explicit_obj_col_with_dt'])
@settings(max_examples=20, deadline=None)
@given(test_df=data())
def test_cat_columns(test_df, selected_col, exp_output):
    ...
    if not exp_output == 'evaluate':
        ...
        if (len(test_df_.groupby([new_col_nm], sort=False).count()) == 1) & (test_df_[new_col_nm][0] in [str]) :
            expected_list = [old_col_nm]
...
def make_indexed_data(draw, segment_by):
    ...
    def get_sampling_list(col_num, row_size):
        return ['Seg'+str(col_num)+'_Type'+str(i) for i in range(row_size)]+NA_VALUES
    df_dict = {}
    for col in range(col_len):
        col_name = 'col_'+str(col)
        df_dict[col_name] = draw(lists(elements=sampled_from(get_sampling_list(col, int(row_len/2))),
                                       min_size=row_len, max_size=row_len))
    df = pd.DataFrame(df_dict)
    df['segments'] = df.apply(lambda x: '_'.join([str(x[col_nm]) for col_nm in segment_by]), axis=1)
    return df
...
def test_calculate_all_segments(test_df, no_of_segments):
    ...
    #computed_list = an_obj._calculate_all_segments()
