...
def compute_pr_curve(y, yhat, multi_class=False, classes=[]):
    ...
    if not multi_class:
        ...
        precision_["micro"], recall_["micro"], _ = precision_recall_curve(y.ravel(), yhat.ravel())
        score_["micro"] = average_precision_score(y, yhat, average="micro")
    if type(precision_) == dict:
        plot_data = []
        for i in range(len(classes)):
            plot_data += [pd.DataFrame({"precision": precision_[i], "recall": recall_[i]})]
...
def create_pr_curve(y, yhat, multi_class=False, baseline=True, classes=[], **kwargs):
    ...
    if type(plot_data) == list:
        ...
        for i in range(len(plot_data)):
            pr_curve_list += [
                hvPlot(plot_data[i], **kwargs).line(x="recall", y="precision", label="PRCurve for class " + str(i), ylim=(0, 1),)
            ]
        ...
        if "label" in kwargs:
            label = kwargs["label"]
        pr_curve = hvPlot(plot_data, **kwargs).line(x="recall", y="precision", label=label, ylim=(0, 1),)
    if baseline:
        avg_p = hv.Curve(pd.DataFrame.from_dict({"x": [0, 1], "y": [avg_precision] * 2}), label="avg precision")
...
def gains_table(y, yhat, cut_off_points=deciles_, custom_lift_data=None, show_lift=True, show_mean=False):
    ...
    out_df["targets_in_bucket"] = out_df["true_positive"] / (out_df["true_positive"] + out_df["false_positive"])
    out_df["percent_targets_captured"] = out_df["true_positive"] / pop_events
    out_df["lift"] = out_df["targets_in_bucket"] / prevalence
    if custom_lift_data is not None:
        for col in custom_lift_data.columns:
            if show_lift:
                out_df["custom_lift_"+col] = out_df["mean_"+col] / float(df["lift_metric_"+col].mean())
            if not show_mean:
...
def gains_chart(df_gains_table, baseline=True, **kwargs):
    ...
    gains_data = df_gains_table["percent_targets_captured"].reset_index(drop=True).to_frame()
...
def lift_chart(df_gains_table, baseline=True, **kwargs):
    ...
    lift_curve = hvPlot(lift_data).line(y="lift", x="deciles", ylim=(1, max(lift_data["lift"])), **kwargs)
    if baseline:
...
def compute_roc_curve(y, yhat, multi_class=False, classes=None):
    ...
    if not (multi_class):
        ...
        index = 1  # Get the curve for +ve class
    ...
    plot_data = pd.DataFrame({"True Positive Rate": tpr["macro"], "False Positive Rate": fpr["macro"],})
...
def create_roc_curve(y, yhat, label="LogisticRegression", **kwargs):
    ...
    plot = hvPlot(plot_data).line(
        ...
        xlim=(0, 1),
        label="ROC for {}, AUC: {}".format(label, round(roc_auc, 3), legend="right"),
        **kwargs,
    )
    return plot
...
def create_threshold_chart(y, yhat, **kwargs):
    ...
    for col in plot_data.columns:
        if x == col:  # Do not plot threshold
            continue
        current_plot = hvPlot(plot_data).line(x=x, y=col, xlim=(0, 1), ylim=(0, 1), label=col, **kwargs,)
        if plot is None:
            plot = current_plot
        else:
...
class ClassificationEvaluation(Evaluator):
    def __init__(
        ...
        super().__init__(model, x_train, y_train, x_test, y_test, yhat_train, yhat_test, display_labels)
        ...
        if not self.multi_class:
            ...
            if any(pd.DataFrame(self.yhat_train).nunique() > 2):
                self.plots['probability_distribution'] = self.get_probability_distribution
    ...
    def precision_recall_curve(self, baseline=True, **kwargs):
        ...
        """
        train_plot = create_pr_curve(self.y_train, self.yhat_train, baseline=baseline, **kwargs).opts(
            title="Train Data", frame_width=350, frame_height=300
        )
        if self.has_test:
            test_plot = create_pr_curve(self.y_test, self.yhat_test, baseline=baseline, **kwargs).opts(
    ...
    def _get_classification_report(self, y_true, yhat):
        ...
        cr = cr.loc[cr["#"].isin(["accuracy", "macro avg", "weighted avg"]) == False]
        cr.reset_index(drop=True, inplace=True)
        cr = hvPlot(cr).table()
        return cr

    def get_classification_report(self):
        train_report = self._get_classification_report(self.y_train, self.yhat_train).opts(title="Train Data")
        if self.has_test:
            test_report = self._get_classification_report(self.y_test, self.yhat_test).opts(title="Test Data")
            # cr_dict = {}
    ...
    def _get_class_distributions(self, y_true, yhat):
        ...
        plot_pred = df_cd["Predicted"].hvplot.bar(color='b', alpha=.5, line_color=None)
        cd_plot = plot_true * plot_pred
        cd_plot = cd_plot.opts(frame_width=350, frame_height=300, legend_position='top_right')
        return cd_plot

    # @fail_gracefully
    def get_class_distributions(self):
        train_plot = self._get_class_distributions(self.y_train, self.yhat_train).opts(title="Train Data",
                                                                                       legend_position='right')
        if self.has_test:
            test_plot = self._get_class_distributions(self.y_test, self.yhat_test).opts(title="Test Data",
                                                                                        legend_position='right')
            # cd_dict = {}
    ...
    def _compute_cm(self, y_true, y_pred, return_df=True):
        ...
        if self.multi_class:
            cm_df.rename(columns={i: "Actual_" + self.display_labels[i] for i in self.display_labels}, inplace=True)
            cm_df.rename(index={i: "Predicted_" + self.display_labels[i] for i in self.display_labels}, inplace=True)
        else:
    ...
    def confusion_matrix(self, normalized=False, flattened=False):
        ...
        if self.has_train:
            ...
            cms[TRAIN_PREFIX] = self._compute_cm(self.y_train, y_pred_train, return_df=False)
        if self.has_test:
            y_pred_test = self.yhat_test.copy()
            y_pred_test[y_pred_test >= 0.5] = 1
            y_pred_test[y_pred_test < 1] = 0
            cms[TEST_PREFIX] = self._compute_cm(self.y_test, y_pred_test, return_df=False)
        for dataset in cms:
            if normalized:
                cms[dataset] = (
    ...
    def get_confusion_matrix(self,cutoff_value=0.5):
        train_plot = self._get_conf_matrix(self.y_train, self.yhat_train).opts(title="Train Data", width=350, height=100)
        if self.has_test:
            test_plot = self._get_conf_matrix(self.y_test, self.yhat_test).opts(title="Test Data", width=350, height=100)
            cm_dict = {}
            for thresh in range(5, 100, 5):
                cm_dict.update({thresh / 100: hv.Layout([train_plot[thresh/100], test_plot[thresh/100]])})
    ...
    def _get_probability_distribution(self, y_true, yhat):
        ...
        for key in cm_df["Actual-Predicted"].unique():
            cm_plot[key] = cm_df.loc[cm_df["Actual-Predicted"] == key].hvplot.box(y=cols, groupby=["Actual-Predicted"], ylabel='Probability')
    ...
    def threshold_curve(self, **kwargs):
        ...
        """
        train_plot = create_threshold_chart(self.y_train, self.yhat_train, **kwargs).opts(
            title="Train Data", frame_width=350, frame_height=300
        )

        if self.has_test:
            test_plot = create_threshold_chart(self.y_test, self.yhat_test, **kwargs).opts(
                title="Test Data", frame_width=350, frame_height=300
            )
            return (train_plot + test_plot).cols(2)
        return train_plot
    ...
    def get_metrics(self, vary_thresholds=True, cutoff_value=0.5):
        ...
        if hasattr(self.model, "predict_proba") and not self.multi_class and vary_thresholds:
            metrics_dict = {}
            from tigerml.core.plots import hvPlot
            for thresh in range(5, 100, 5):
                metrics = self._get_metrics_for_decision_threshold(threshold=thresh / 100)
                try:
                    metrics = metrics.transpose().reset_index(level=[1]).pivot(columns="dataset")[0].reset_index()
                except:
                    import pdb
                    pdb.set_trace()
    ...
    def gains_chart(self, baseline=True, **kwargs):
        self._init_gains()
        train_plot = gains_chart(self.gains_table_result_train, baseline=baseline, **kwargs).opts(
            title="Train Data", frame_width=350, frame_height=300
        )
        if self.has_test:
            test_plot = gains_chart(self.gains_table_result_test, baseline=baseline, **kwargs).opts(
                title="Test Data", frame_width=350, frame_height=300
            )
            return (train_plot + test_plot).cols(2)
        return train_plot
    ...
    def lift_chart(self, baseline=True, **kwargs):
        self._init_gains()
        train_plot = lift_chart(self.gains_table_result_train, baseline=baseline, **kwargs).opts(
            title="Train Data", frame_width=350, frame_height=300
        )
        if self.has_test:
            test_plot = lift_chart(self.gains_table_result_test, baseline=baseline, **kwargs).opts(
                title="Test Data", frame_width=350, frame_height=300
            )
            return (train_plot + test_plot).cols(2)
        return train_plot
    ...
    def get_plots(self, cutoff_value=0.5):
        ...
        if not self.multi_class:
            ...
            plots_dict["confusion_matrix"] = [self.get_confusion_matrix(cutoff_value=cutoff_value)]
        else:
            plots_dict["confusion_matrix"] = self.get_confusion_matrix_multiclass(cutoff_value=cutoff_value)
...
class ClassificationComparisonMixin:
    def perf_metrics(self, cutoff_value=0.5):
        ...
        for model_name in self.reporters:
            current_metrics = self.reporters[model_name].evaluator.get_metrics(vary_thresholds=False, cutoff_value=cutoff_value)
        ...
        table.apply_conditional_format(
            cols=[x for x in perf_metrics.columns if all([col not in x for col in bad_metrics])],
            style=preset_styles.more_is_good_2colors,
        )
        table.apply_conditional_format(
            cols=[x for x in perf_metrics.columns if any([col in x for col in bad_metrics])],
    ...
    def roc_curves(self):
        ...
        roc_curves = None
        for model_name in self.reporters:
            current_roc = self.reporters[model_name].evaluator.roc_curve(label=model_name)
    ...
    def confusion_matrices(self):
        ...
        cm_dict = {}
        for thresh in range(5, 100, 5):
            cm_dict.update({thresh / 100: hv.Layout([cm_matrices[model][thresh/100] for model in cm_matrices])})
    ...
    def pr_curves(self):
        ...
        for model_name in self.reporters:
            if model_name == list(self.reporters.keys())[-1]:
                baseline = True
            current_pr = self.reporters[model_name].evaluator.precision_recall_curve(label=model_name, baseline=baseline)
    ...
    def threshold_analysis(self):
        """Returns line plot with `precision recall`, `f1 score` and `prevalence` as `threshold` is varied for all the
        input models.
    ...
    def gains_charts(self):
        ...
        for model_name in self.reporters:
            ...
            if model_name == list(self.reporters.keys())[-1]:
                baseline = True
            current_gains_curve = evaluator.gains_chart(baseline=baseline, label=model_name)
    ...
    def lift_charts(self):
        ...
        for model_name in self.reporters:
            ...
            if model_name == list(self.reporters.keys())[-1]:
                baseline = True
            current_lift_curve = evaluator.lift_chart(baseline=baseline, label=model_name)
    ...
    def multi_classification_reports(self):
        multi_cls_reports = {}
        for model_name in self.reporters:
            multi_cls_reports[model_name] = self.reporters[model_name].evaluator.get_classification_report()
        return multi_cls_reports

    def multi_classification_distributions(self):
        multi_cls_dists = {}
        for model_name in self.reporters:
            multi_cls_dists[model_name] = self.reporters[model_name].evaluator.get_class_distributions()
        return multi_cls_dists

    def multi_classification_cm(self):
        cm_matrices = {}
        for model_name in self.reporters:
            cm_matrices[model_name] = self.reporters[model_name].evaluator.get_confusion_matrix_multiclass()
        return cm_matrices
    ...
    def get_performance_report(self, cutoff_value=0.5):
        ...
        perf_dict = {}
        if not self.multi_class:
            perf_dict["performance_metrics"] = [self.perf_metrics(cutoff_value=cutoff_value)]
            ...
            perf_dict["class_distributions"] = [self.multi_classification_distributions()]
        return perf_dict
